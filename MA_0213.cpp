...
int nid[maxn];
bool del[maxn], visited[maxn];
vector<int> component[maxn];
vector<int> parent[parent_num + 10];
vector<int> S, S0, S_best, tmpS;
stack<int> unused_nid;
set<int> used_nid;
int f_best, f_avg, succ, exch;
...
vector<int> total_distance(parent_num + 1, 0);
...
void PoolInitialize() {
	...
	for (int i = 0; i < parent_num; ++i) {
		...
		parent[i] = S;connectivities[i] = Connect();
		for(int j = 0; j < i; ++j)
			distances[j][i] = (distances[i][j] = get_distance(parent[i], S));//初始化距离矩阵，get_distance也还可以优化为，总是求同一个解与其他不同的解时的距离时
		for (auto &num:S) del[num]=false;
	}
	vector<int> v_connectivity = connectivities;
	...
	for(int i = 0; i < parent_num; ++i) {//初始化p_distance
		for(int j = 0; j < parent_num; ++j) 
			if(j != i) total_distance[i] += distances[i][j];
	}
}
...
void RankBasedPoolUpdating(int connectivity) {//尝试不要直接加进去再算要不要加，而是根据原本的解，构建一个阈值，不用每次计算这么多
	parent[parent_num] = S;
	...
	rank_connectivity[parent_num] = parent_num - count + 1;
	int idx = -1;
	double max_score = -1;
	for(int i = 0; i < parent_num; ++i) {
		int dis = get_distance(S, parent[i]);
		distances[parent_num][i] = dis;
		distances[i][parent_num] = dis;
		total_distance[parent_num] += dis;
		total_distance[i] += dis;
	}//get_distance也还可以优化为，总是求同一个解与其他不同的解时的距离时
	vector<int> v_distance = total_distance;
	...
	for(int i = 0; i < parent_num + 1; ++i) {
		...
		double score = BETA * rank_connectivity[i] + (1 - BETA) * distance(it, v_distance.begin());//返回该total_distance在排序后的数组中的迭代器
		if(score > max_score) {
			idx = i;
			max_score = score;
		}
	...
	if(idx != parent_num) {//total_distance需要更新，distances上三角与下三角都要更新
		...
		total_distance[idx] = total_distance[parent_num];
		total_distance[parent_num] = 0;
		for(int i = 0; i < parent_num; ++i) 
			if(i != idx)
				total_distance[i] -= delete_distances[i];
			else total_distance[i] -= delete_distances[parent_num];
...
void solve() {
	...
	while (!timeout()) {
		...
		//cout << S[0] << " " << S[1] << endl;
		tmp = Connect();
		cout << "find solution objective value so far : " << best_connectivity << endl;
		if (tmp <= kbv) {
			cout << "find best solution " << kbv << endl;
			cout << "time_cost : " << time_cost() << endl;
			break;
		}
		if (tmp < best_connectivity) {
			S0 = S;
