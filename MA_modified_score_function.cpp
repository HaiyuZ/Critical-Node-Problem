...
void read(string filename) {
	...
	for (int i = 0; i < vertex_num; ++i) {
		...
		//adjlist[i] = new int[v.size()];
		//for(int j = 0; j < v.size(); ++j) adjlist[i][j] = v[j];
		//vertex_degree[i] = v.size();
		//v.clear();
	}
}
...
/*
int insert_pos(vector<int> nums, int num) {
	...
	while(left <= right) {
		mid = (left + right) / 2;
		if(nums[mid] == num) {
			while(mid + 1 < nums.size() && nums[mid + 1] == num) mid++;
			return mid + 2; 
		}
...
void RankBasedPoolUpdating(int connectivity){
	...
	for(int i = 0; i < parent_num; ++i)
		if(connectivities[i] > connectivity) rank_connectivity[i]++;
...
int main() {
	K = 50;
	string filename = "BarabasiAlbert_n500m1.txt";
	read(filename);
	//show_adjlist();
	for(int i = 0; i < parent_num + 1; ++i) distances[i] = new int[parent_num + 1];
	time_out=60;
	run_times=1;
